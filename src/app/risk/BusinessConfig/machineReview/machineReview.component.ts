...
@Component({
  selector: "",
  templateUrl: "./machineReview.component.html",
  styleUrls: ['./machineReview.component.less']
})
export class MachineReviewComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.validateForm = this.fb.group({
      ...
      checkParameters: [null, [Validators.required]],
          ...
          filter:(item)=>{
            if (item['checkParameters'] != null) {
              ...
              checkParameters.forEach(v => {
                let name = map.filter(cc => {
                  return cc['id'] == v;
                });
                str = (name.length > 0 && name[0]['paramName']) ? str + '，'+v+"."+ name[0]['paramName'] : ""
              });
    ...
    this.service.getReview(data)
      .pipe(
        filter((res: Response) => {
          ...
          if (res.data && res.data['length'] == 0) {
            this.selectDataBy = this.languagePack['data']['stageDataBy'];
            this.selectDataRefuse = this.languagePack['data']['stageDataRefuse'];
      ...
      .subscribe(
        (res: Response) => {
          ...
          this.selectDataBy = this.selectData.concat(this.languagePack['data']['stageDataBy']);
          this.selectDataRefuse = this.selectData.concat(this.languagePack['data']['stageDataRefuse']);
