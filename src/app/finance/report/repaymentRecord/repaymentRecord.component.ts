...
import {repaymentRecordService} from '../../../service/fincial';
...
let __this;
@Component({
  selector: 'repayment-record',
  templateUrl: './repaymentRecord.component.html',
  styleUrls: ['./repaymentRecord.component.less']
})
export class RepaymentRecordComponent implements OnInit {
  constructor(
    private translate: TranslateService,
    private service: repaymentRecordService,
  ...
  getLanguage() {
    this.translate.stream(['common', 'finance.report.repaymentRecord'])
      .subscribe(
        res => {
          this.languagePack = {
            common: res['common'],
            data: res['finance.report.repaymentRecord'],
            table: res['finance.report.repaymentRecord']['table']
          };
    ...
    this.tableData = {
      ...
      tableTitle: [
        ...
        },
        {
          ...
          type: 'text',
          filter : (item) => {
            return item.currentPeriod==null ? "" : (item.currentPeriod+"/"+(item.totalPeriod==null ? 1 : item.totalPeriod));
          }
        },
        {
          ...
          type: 'text',
          filter : (item) => {
            let status=item['paymentMethod'];
            if(status!==null){
              let type=__this.languagePack.data.payMethods.filter(v => {
        ...
        {
          name: __this.languagePack['table']['description'],
          reflect: 'description',
          type: 'text'
        },
        {
      ...
      //     bindFn: (item) => {
      //       //实现单行刷新功能
      //       this.service.getList({"id":item.id})
      //       .subscribe(
      //         (res: Response) => {
  ...
  downloandRepaymentList(){
    ...
    this.service.downloandRepaymentList(this.searchModel);
