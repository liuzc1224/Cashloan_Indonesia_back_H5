...
export class ContactInfoComponent implements OnInit{
  ...
  ngOnInit(){
    ...
    this.validForm = this.fb.group({
      ...
      "contactWay" : [null , [Validators.required] ] ,
  ...
  getLanguage(){
    this.translateSer.stream(["productCenter.appConfig.contactInfo","common"])
      .subscribe(
        data => {
          this.languagePack = {
            common : data['common'] ,
            data:data['productCenter.appConfig.contactInfo'],
          };
  ...
  initialTable(){
    this.tableData = {
      ...
      tableTitle : [
        ...
        {
          name : __this.languagePack['data']['contact'] ,
          reflect : "contactWay" ,
          type : "text"
        },
        {
        ...
        },
        {
          ...
          type : "text",
          filter(item){
            return item.status=="1"?__this.languagePack['data']['status'][1]['desc']:__this.languagePack['data']['status'][2]['desc']
          }
        }
      ] ,
      btnGroup : {
  ...
  getList(){
    ...
    this.service.getContactInfo(data)
      .pipe(
        filter( (res : Response) => {
  ...
  handleOk(){
    ...
    if(postData["id"]){
      this.updateContactInfo(postData);
    }
  }
  updateContactInfo(data){
    this.service.updateContactInfo(data)
      .pipe(
        filter( (res : Response) => {
          this.isOkLoading = false;
          if(res.success === false){
